# -*- coding: utf-8 -*-
"""
Created on Fri Dec 19 06:40:30 2025
...
exp_wavelength = 561e-9;
exp_z_dist = 24.3e-3;#24.3e-3;#24.3e-3 - 0.4e-3;#29.03e-3;#24.3e-3;#26.9e-3;#24.06e-3;#24.39e-3;#24.06e-3;#27.3e-3;#26.824e-3;
...
class Correlated_Speckle_Imaging(nn.Module):
    def __init__(self, params, speckle_data):
        ...
        self.scanning_positions[:,0] = (1 - 0.) * self.scanning_positions[:,0];#self.scanning_positions0[:,0];#
        self.scanning_positions[:,1] = (1 - 0.) * self.scanning_positions[:,1];#self.scanning_positions0[:,1];#
    ...
    def Fres_propagate_test(self, field, object_spatial_res, wavelength, z, pad_factor):
        ...
        """
        Use direct implementation of FFT to compute the Fresnel-propagaed field of the input-variable >field<
        ...
        padded_field = nn.functional.pad(
            field, (0, int(nx * (pad_factor - 1)), 0, int(ny * (pad_factor - 1)))
        )
    ...
    def optimize(
        ...
        for iteration in pbar:
            ...
            for idx in indices:
                ...
                ) #+ 300e-6 * ((torch.sum(torch.abs(self.object - torch.roll(self.object, 1, 0)))
...
#        )
        ...
        # experiment_data = np.load(os.path.join("C:\\Users\\janbr\\Downloads\\Speckle_Imaging_Piezo_Nanocube_Siemens_star_Reconstr_Data_speckle_NA_close_diffuser_Blocked_background_Fixed_Position_100_nm_y_dir_sweep_5_50_nm_5_nm_step_x_dir_20032026", "organoid_scan_20260320_121759.npz"),
               #allow_pickle=True)
        experiment_data = np.load("C:\\Users\\lotte\\OneDrive\\Documenten\\Universiteit\\BSc\\Scriptie\\Reconstructie\\updatedCode\\organoid_scan_20260605_181053.npz", allow_pickle=True)
        
        scanning_positions = 1 * experiment_data["scan_positions"][::,:] * 1e-6
        
        

        scanning_positions[:,1] = -scanning_positions[:,1]# + 100e-9 * 44.44;
        #scanning_positions[:,1] = scanning_positions[:,1] - 100e-9 * 44.44;#- 200e-9 * 44.44;
        ...
        dist_des = 0.005;#0.0005;#0.0005;#0.0005;#0.0006;#0.00035;
...
#%% OPTIONAL: Section intended to smooth-out camera-images from eventual high-freq. fringes occurrence from interference-artefacts

N_inds_smooth = 5;


exp_data_images0_smooth = np.zeros(exp_data_images0.shape + np.array([0, 2 * N_inds_smooth, 2 * N_inds_smooth]));#, 1, axis = 1), 1, axis = 2);
#%% OPTIONAL: Section intended to smooth-out camera-images from eventual high-freq. fringes occurrence from interference-artefacts


for i in range(-N_inds_smooth, N_inds_smooth + 1):
    for j in range(-N_inds_smooth, N_inds_smooth + 1):
        print(j)
        Sm_factor_exp = np.exp(-1 / (2 * 2 ** 2) * (i ** 2 + j ** 2));
        exp_data_images0_smooth[:,N_inds_smooth + i:exp_data_images0.shape[1] + N_inds_smooth + i, N_inds_smooth + j:exp_data_images0.shape[2] + N_inds_smooth + j] += Sm_factor_exp * exp_data_images0;

#%% OPTIONAL: Section intended to smooth-out camera-images from eventual high-freq. fringes occurrence from interference-artefacts


Sm_norm_const_exp = 0.0;

for i in range(-N_inds_smooth, N_inds_smooth + 1):
    for j in range(-N_inds_smooth, N_inds_smooth + 1):
        print(j)
        Sm_norm_const_exp += np.exp(-1 / (2 * 2 ** 2) * (i ** 2 + j ** 2));
#%
exp_data_images0_smooth = 1 / (Sm_norm_const_exp) * exp_data_images0_smooth;
#%
exp_data_images0 = exp_data_images0_smooth[:,N_inds_smooth:-N_inds_smooth,N_inds_smooth:-N_inds_smooth];
...
Model_obj_init_store_100_nm_x_nonshift_prec = model.object.detach();
Model_diff_init_store_100_nm_x_nonshift_prec = model.diffuser.detach();



#%%

Model_obj_init_store_100_nm_x_shifted = model.object.detach();
Model_diff_init_store_100_nm_x_shifted = model.diffuser.detach();


#%%
Model_obj_init_store_100_nm_x_shifted_prec = model.object.detach();
Model_diff_init_store_100_nm_x_shifted_prec = model.diffuser.detach();

#%%

model_object_smoothed_np_array = np.zeros(np.array(model.object.shape), dtype = 'complex128');
...
for i in range(-N_inds_smooth, N_inds_smooth + 1):
    for j in range(-N_inds_smooth, N_inds_smooth + 1):
        print(j)
        Sm_factor_exp = np.exp(-1 / (2 * 2 ** 2) * (i ** 2 + j ** 2));
        model_object_smoothed_np_array += np.roll(np.roll(np.array(model.object.detach()), i, 0), j, 1);

#%% Everything above this cell by JBMY Heinisch, below by L Gritter
# Cropping suplots
# crop to a region around the centre of the object/diffuser
center_object = model.object.shape[0] // 2
center_diffuser = model.diffuser.shape[0] // 2
# set the size of the cropped image
height_object = 300
height_diffuser = 300

# crop and plot
cropped_object = model.object[center_object-height_object:center+height_object, center_object-height_object:center_object+height_object]
model.plot_complex_subplots(cropped_object, "Reconstructed Object (cropped)")

cropped_diffuser = model.diffuser[center_diffuser-height_diffuser:center_diffuser+height_diffuser, center_diffuser-height_diffuser:center_diffuser+height_diffuser]
model.plot_complex_subplots(cropped_diffuser, "Reconstructed Diffuser (cropped)")
...
def fisher_info(shift, difference):
    ...
    #Fisher information for pixel k
    Fisher_information_tensor = torch.div( torch.mul(dI_dx, dI_dx), I0)
    Fisher_information_tensor_crop = Fisher_information_tensor[300:500, 300:500]
    Fisher_information = torch.sum(Fisher_information_tensor)
    # print("This is dI_dx")
    # print(dI_dx)
    return Fisher_information, dI_dx

print(fisher_info([0,0],[0,0.00002])[0].item())
...
# Fisher information matrix A
A = torch.tensor(fisher_info([0,0],[0,5e-9],[5e-9,0]))
# Some print statements to check whether it performed as expected
print(A)
print(A[0][1])
# Calculate the inverse (for the CRLB)
print(torch.linalg.inv(A))

#%% Angle test
# Section to test whether the angle dependence behaves as expected
step = 5e-9
...
# Plot the diagonal and off-diagonal components as a function of the angle
plt.plot(t, F10)
plt.plot(t, F00)
plt.plot(t,Fdif)
plt.xlabel("Angle")
...
#Seems to work abonve 0.00002
# %% Check for are outliers in data
